...
Definition queue_invariant (Ψ : val -> iProp Σ) (l_head l_tail : loc) (Q_γ : Qgnames) : iProp Σ :=
	∃ xs_v, (* Abstract state *)
	∃ xs xs_rest xs_old (x_head x_tail: (loc * val * loc)), (* Concrete state *)
	⌜xs = xs_rest ++ [x_head] ++ xs_old⌝ ∗
	isLL xs ∗
	⌜proj_val xs_rest = wrap_some xs_v⌝ ∗
	All xs_v Ψ ∗
	(
		(* Static *)
		(
...
Proof.
	...
	iSplit.
	- iIntros "(%xs_v & %xs & %xs_rest & %xs_old & %x_head & %x_tail & Hxs_split & HisLL_xs & %Hconc_abst_eq & HAll & [H_Static | [H_Enqueue | [H_Dequeue | H_Both]]])"; 
	iExists xs_v, xs, xs_rest, xs_old, x_head, x_tail; iFrame.
	  ...
	  + iDestruct "H_Both" as "(Hl_head & Hl_tail & [[HisLast HTokAfter] | [HisSndLast HAfter]] & HToke & HTokD)".
		...
		* iSplit; first done.
		  iSplitL "Hl_head HTokD"; first (iRight; iFrame). 
		  iRight. iFrame. iRight. iFrame.
	- iIntros "(%xs_v & %xs & %xs_rest & %xs_old & %x_head & %x_tail & Hxs_split & HisLL_xs & %Hconc_abst_eq & HAll & [[Hl_head HToknD] | [Hl_head HTokD]] & [(Hl_tail & HisLast & HToknE & HTokUpdated) | (Hl_tail & HTokE & [[HisLast HTokBefore] | [HisSndLast HTokAfter]])])";
	iExists xs_v, xs, xs_rest, xs_old, x_head, x_tail; eauto 10 with iFrame.
Qed.
...
Proof.
	...
	iDestruct "H_queue_inv_open" as "(%xs_v & %xs & %xs_rest & %xs_old & %x_head & %x_tail & >%H_eq_xs & HisLL_xs & >%Hconc_abst_eq & HAll_xs & H_head & [ ( [Hl_tail1 Hl_tail2] & >[%xs_fromtail %H_isLast] & HToknE & HTokUpdated ) | (_ & >HTokE2 & _) ])"; last by iCombine "HTokE HTokE2" gives "%H". (* Impossible: TokE *)
	wp_load.
	iModIntro.
	...
	iSplitL "H_head Hl_tail1 HTokE HTokBefore HisLL_xs HAll_xs".
	{
		...
		iExists xs_v, xs, xs_rest, xs_old, x_head, x_tail.
		iFrame.
	...
	iInv "H_queue_inv" as "H_queue_inv_open".
	iPoseProof (queue_invariant_equiv_simple Ψ l_head l_tail qg with "H_queue_inv_open") as "H_queue_inv_open".
	iDestruct "H_queue_inv_open" as "(%xs_v_2 & %xs_2 & %xs_rest_2 & %xs_old_2 & %x_head_2 & %x_tail_2 & >%H_eq_xs_2 & HisLL_xs_2 & >%Hconc_abst_eq_2 & HAll_xs_2 & H_head & [ ( _ & _ & >HToknE2 & _ ) | (>Hl_tail1 & HTokE & [[>[%xs_fromtail_2 %H_isLast_2] HTokBefore] | [_ >HTokAfter2]]) ])"; 
	...
	iSplitL "H_head Hl_tail1 HTokE HTokAfter HisLL_xs_new HAll_xs_2 HΨ_v".
	{
		iNext.
		iApply queue_invariant_equiv_simple.
		iExists (v :: xs_v_2), xs_new, (x_new :: xs_rest_2), xs_old_2, x_head_2, x_tail.
	...
	iInv "H_queue_inv" as "H_queue_inv_open".
	iPoseProof (queue_invariant_equiv_simple Ψ l_head l_tail qg with "H_queue_inv_open") as "H_queue_inv_open".
	iDestruct "H_queue_inv_open" as "(%xs_v_3 & %xs_3 & %xs_rest_3 & %xs_old_3 & %x_head_3 & %x_tail_3 & >%H_eq_xs_3 & HisLL_xs_3 & >%Hconc_abst_eq_3 & HAll_xs_3 & H_head & [ ( _ & _ & >HToknE2 & _ ) | (>Hl_tail1 & HTokE & [[_ >HTokBefore2] | [>(%x_new_2 & %xs_fromtail_3 & %H_isSndLast) HTokAfter]])])"; 
	[ by iCombine "HToknE HToknE2" gives "%H" | (* Impossible: ToknE *)
	...
	iSplitR "HTokE Hlocked_γ_Tlock HΦ".
	{
		iNext.
		iApply queue_invariant_equiv_simple.
		iExists xs_v_3, xs_3, xs_rest_3, xs_old_3, x_head_3, x_new.
		iSplit; first done.
		iFrame.
		iSplit; first done.
		iLeft.
...
Proof.
	...
	iInv "H_queue_inv" as "H_queue_inv_open".
	iPoseProof (queue_invariant_equiv_simple Ψ l_head l_tail qg with "H_queue_inv_open") as "H_queue_inv_open".
	iDestruct "H_queue_inv_open" as "(%xs_v & %xs & %xs_rest & %xs_old & %x_head & %x_tail & >%H_eq_xs & H_isLL_xs & >%Hconc_abst_eq & HAll_xs & [ [Hl_head HToknD] | [Hl_head >HTokD2] ] & H_tail)"; 
	...
	iInv "H_queue_inv" as "H_queue_inv_open".
	iPoseProof (queue_invariant_equiv_simple Ψ l_head l_tail qg with "H_queue_inv_open") as "H_queue_inv_open".
	iDestruct "H_queue_inv_open" as "(%xs_v_2 & %xs_2 & %xs_rest_2 & %xs_old_2 & %x_head_2 & %x_tail_2 & >%H_eq_xs_2 & H_isLL_xs_2 & >%Hconc_abst_eq_2 & HAll_xs_2 & [ [Hl_head >HToknD2] | [>Hl_head2 HTokD] ] & H_tail)";
	...
	- (* Queue is non-empty*)
	  ...
	  {
		iNext. iApply queue_invariant_equiv_simple.
		iExists xs_v_2, ((xs_rest_2'' ++ [x_head_next]) ++ [x_head] ++ xs_old_2), (xs_rest_2'' ++ [x_head_next]), xs_old_2, x_head, x_tail_2.
	  ...
	  iInv "H_queue_inv" as "H_queue_inv_open".
	  iPoseProof (queue_invariant_equiv_simple Ψ l_head l_tail qg with "H_queue_inv_open") as "H_queue_inv_open".
	  iDestruct "H_queue_inv_open" as "(%xs_v_3 & %xs_3 & %xs_rest_3 & %xs_old_3 & %x_head_3 & %x_tail_3 & >%H_eq_xs_3 & H_isLL_xs_3 & >%Hconc_abst_eq_3 & HAll_xs_3 & [ [Hl_head >HToknD2] | [>Hl_head2 HTokD] ] & H_tail)";
	  first by iCombine "HToknD HToknD2" gives "%H". (* Impossible ToknD *)
	  ...
	  rewrite Hxs_rest_eq_3 in Hconc_abst_eq_3.
	  rewrite (proj_val_split xs_rest_3'' [x_head_next_3]) in Hconc_abst_eq_3.
	  rewrite (wrap_some_split xs_val_rest [x_head_next_val]) in Hconc_abst_eq_3.
	  simpl in Hconc_abst_eq_3.
	  apply (list_last_eq (proj_val xs_rest_3'') (wrap_some xs_val_rest) (n_val x_head_next_3) (InjRV x_head_next_val)) in Hconc_abst_eq_3 as [Hxs_rest_val_eq Hx_head_next_eq].
	  ...
	  {
		iNext. iApply queue_invariant_equiv_simple.
		iExists xs_val_rest, (xs_rest_3'' ++ [x_head_next_3] ++ [x_head] ++ xs_old_3), xs_rest_3'', ([x_head] ++ xs_old_3), x_head_next_3, x_tail_3.
