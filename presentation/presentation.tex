...
\begin{frame}[fragile]{Implementation: $\dequeue$}
  \begin{itemize}
    \item The \textcolor{Brown}{$\dequeue$} function \textcolor{blue}{checks} if queue is \textcolor{blue}{empty}
      ...
      \end{itemize}
    \item Dequeued node \textcolor{red}{not freed} $\rightarrow$ Linked list \textcolor{blue}{only grows}
  \end{itemize}
  \vspace{-8pt}
  \begin{minted}[escapeinside=||, mathescape=true, bgcolor=codebg, frame=lines, fontsize=\footnotesize]{ocaml}
|$ \dequeue \ Q \eqdef $|
...
\begin{frame}{Invariant}
  ...
  \begin{itemize}
    ...
    \item Defines \textcolor{blue}{structure} of the \textcolor{blue}{concrete} linked list, $xsc$
    \item Asserts \textcolor{blue}{relation} between \textcolor{blue}{abstract state} and \textcolor{blue}{concrete state}
    \item Identifies possible \textcolor{blue}{queue states}: \StaticState, \EnqueueState, \DequeueState, and \BothState{}
      \begin{itemize}
...
\begin{frame}{Queue Predicate}
  ...
  \begin{itemize}
    \item The queue predicate is \textcolor{blue}{persistent}, as all its constituents are
    \item \textcolor{blue}{Proving} that \tlmsq{} satisfies the \textcolor{blue}{HOCAP-style specification} then consists of proving the \textcolor{blue}{Hoare triples} for \textcolor{Brown}{$\initialise$}, \textcolor{Brown}{$\enqueue$}, and \textcolor{Brown}{$\dequeue$}
    \item We here \textcolor{blue}{focus} on \textcolor{Brown}{$\enqueue$}
  \end{itemize}
\end{frame}
