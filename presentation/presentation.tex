...
%=====================================================================
\begin{frame}{Nature of Specifications}
  \todo{create slide}
  % - written in Iris, a Higher order CSL
  % - In terms of Hoare triples
  % - Partial correctness
  % + In particular, doesn't crash
  % - Clients use the Hoare triples specifications to prove results about their own code
  %   Must obtain resources describe by pre-condition to apply Hoare triple
  %   Obtains resources describe by post-condition after invocation
\end{frame}

%=====================================================================
\begin{frame}{Sequential Specification}
  \todo{create slide}
  % - Basics of queue algorithms: Consist of initialise, enqueue, dequeue
  % - Initialise creates basic data structure. Enqueue adds element. Dequeue pops element, if queue is non-empty. Otherwise returns None.
  % - Show specification
\end{frame}
...
\begin{frame}{HOCAP-style Specification}
  \todo{create slide}
  % - Resource Algebra for Abstract State
  % - Authoritative and Fragmental predicates for tracking abstract state
  % - Specification
\end{frame}

%=====================================================================
\begin{frame}{Queue Client}
  \todo{create slide}
  % - PoC client
  % - Code
  % - Specification
  % - Idea: client is complex enough to require HOCAP
  % + Uses concurrency, so Sequential is not strong enough
  % + Importance of invocations to dequeue not returning None
  % * Concurrent spec only tells us that queue either returns Some or None.
  % * HOCAP allows us to track exact contents
  % * Idea of proof: create invariant that captures possible states of queue contents
  %   Show invariant
\end{frame}
...
%=====================================================================
\begin{frame}{The Lock-and-CC-Free Michael-Scott Queue}
  \todo{create slide}
  % - Show code
\end{frame}
