...
\todo{Emphasise that specs for two-lock and lock-free are Syntactically equivalent, only the interpretation of the queue functions differ}
\todo{Have a bullet point list of contributions}

direct specifications for TL and LF Queues (instead of refinement)
...
\todo{also mention prophecies and refer to section talking about them}
...
\subsection{Persistently Modality}
...
\todo{decide which of the following two sentences makes more sense. Persistent predicate makes sense}
...
\begin{lemma}[Abs-Reach-Alloc]\label{lemma:abs-reach-alloc}
  \todo{change it to proves pvs in the other places for (==*)}
  $\reach{\node}{\node} \proves \pvs (\Exists \gname . \ap{\gname}{\node} \star \ar{\node}{\gname})$
\end{lemma}
\begin{proof}
