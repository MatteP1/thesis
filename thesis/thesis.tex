...
\todo{tying them into the logic}
  ...
  \todo{ghost-allocate own-op, own-valid...}
...
\end{lemma}
\begin{proof}
  This follows directly from ghost-alloc\todo{ref} and the fact that $T$ is valid.
\end{proof}
...
\abstalloc*
\begin{proof}
  By own-op\todo{ref}, it suffices to show $\pvs \Exists \gname . \ownGhost{\gname}{\authfull (1, \aginj ~ \absvalueList) \mtimes \authfrag (1, \aginj ~ \absvalueList)}$. This follows by ghost-alloc\todo{ref} if we can show that the element is valid. That is, $\authfull (1, \aginj ~ \absvalueList) \mtimes \authfrag (1, \aginj ~ \absvalueList) \in \Vl$. However, this follows by the definitions of the involved resource algebras.
\end{proof}
