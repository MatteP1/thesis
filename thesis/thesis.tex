...
We shall use the resource algebra \todo{insert} to achieve the above. List Val\todo{formatting} is the abstract state. It is wrapped in the agreement RA, which ensures that if one owns to elements, then they agree on the abstract state. Q (fraction)\todo{formatting} states how much of the fragmental view is owned; the fragmental view can be split up, which is handled by the clients. The ?\todo{formatting} makes the pair unital\todo{check} which is required by the Auth\todo{formatting} construction. Auth\todo{formatting} is the authoritative resource algebra and gives us the authoritative and fragmental views, and governs that they can only be updated in unison.

For an abstract state $xs_v$ and ghost name gamma, we shall write \todo{insert notation} for the authoritative view, meaning that the authoritative view of the abstract state associated with gamma is $xs_v$. Similarly we write \todo{insert notation} for the fragmental view, when the fraction is $1$.

With this, we now give the hocap-style spec, and explain it afterwards. 

\todo{insert spec}

Firstly, note that we require $\isqueue$ to be persistent, giving us support for concurrency. 

Next, the initialise spec gives clients an additional resource in the postcondition: the ownership of the fragmental view of the empty list, \todo{insert notation}. As discussed above, this allows them to keep track of the contents of the queue.

Finally, the specs for enqueue and dequeue have been parametrised by two predicates $P$ and $Q$. The clients get to pick $P$ and $Q$, and the choice depends on what the client wishes to prove; $P$ describes those resources that the client has before enqueue or dequeue, and $Q$ the resources it will have after. Hence $P$ is in the precondition and $Q$ in the postcondition of the hoare triple. However, before the client gets access to the hoare triple for enqueue or dequeue they must prove a viewshift. This viewshift states how the abstract state of the queue will change as a result of running enqueue or dequeue, and further shows that $P$ can be updated to $Q$. Note that the consequent\todo{or righthand-side?} of the viewshift contains a $\later$. This signifies that the update in the abstract state is tied to a step in the code.
