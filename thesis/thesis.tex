...
\newcommand{\vq}{v_q}
\newcommand{\xs}{xs} % TODO: replace
\newcommand{\xsqueue}{xs_{\mathrm{queue}}}
\newcommand{\xsold}{xs_{\mathrm{old}}} % TODO: replace
...
\newcommand{\tlseqspecinit}{\hoare{\TRUE}{\initialise \TT}{\vq . \Exists Q_\gname. \isqueueseq\ \vq\ []\ Q_\gname}}

\newcommand{\tlseqspecenq}{\All \vq, \absvalue, \absvalueList, Q_\gname. \hoare{\isqueueseq \; \vq \; \absvalueList \; Q_\gname}{\enqueue\ \vq\ \absvalue}{\valB . \isqueueseq\ \vq\ (\absvalue :: \absvalueList)\ Q_\gname}}

\newcommand{\tlseqspecdeq}{\All \vq, \absvalueList, Q_\gname. \hoareV[t]{\isqueueseq \; \vq \; \absvalueList \; Q_\gname}{\dequeue\ \vq}{\nodeval . \begin{array}{l}(\absvalueList = [] \star \nodeval = \None \star \isqueueseq\ \vq\ \absvalueList\ Q_\gname) \; \lor\\ (\Exists \absvalue, \absvalueList' . \absvalueList = \absvalueList' \catenate [\absvalue] \star \nodeval = \Some \absvalue \star \isqueueseq\ \vq\ \absvalueList'\ Q_\gname) \end{array}}}


\newcommand{\tlconcspecinit}{\hoare{\TRUE}{\initialise \TT}{\vq . \Exists Q_\gname. \isqueueconc\ \Phi \ \vq \ Q_\gname}}

\newcommand{\tlconcspecenq}{\All \vq, \absvalue, Q_\gname. \hoare{\isqueueconc \ \Phi \ \vq \ Q_\gname \star \Phi \ \absvalue}{\enqueue\ \vq\ \absvalue}{\valB . \TRUE}}

\newcommand{\tlconcspecdeq}{\All \vq, Q_\gname. \hoare{\isqueueconc \ \Phi \ \vq \ Q_\gname}{\dequeue\ \vq}{\nodeval . \nodeval = \None \lor (\Exists \absvalue. \nodeval = \Some \absvalue \star \Phi \ \absvalue)}}


\newcommand{\tlhocapspecinit}{\hoare{\TRUE}{\initialise \TT}{\vq . \Exists Q_\gname . \isqueue\ \vq \ Q_\gname \star \abstractstatefullfrag{\Qg}{[]}}}

\newcommand{\tlhocapspecenq}{\All \vq, \absvalue, Q_\gname, P, Q.
\begin{array}[t]{l}
...
\wand\\
\hoare{\isqueue \ \vq \ Q_\gname \star P}{\enqueue\ \vq\ \absvalue}{\valB . Q}
\end{array}}

\newcommand{\tlhocapspecdeq}{\All \vq, Q_\gname, P, Q.
\begin{array}[t]{l}
\left(\All \absvalueList . \abstractstateauth{\Qg}{\absvalueList} \star P \vs[\mask\setminus\Nl.i^\uparrow] \later
  ...
  \right) \right)
\wand\\
\hoare{\isqueue \ \vq \ Q_\gname \star P}{\dequeue\ \vq}{\nodeval . Q\ \nodeval}
\end{array}}
...
The proposition $\isqueueseq \; \vq \; \absvalueList \; Q_\gname$ captures that the value $\vq$ is a queue, whose content matches that of our abstract representation $\absvalueList$, and the queue uses the ghost names described by $Q_\gname$. Note that the $\isqueueseq$ predicate is not required to be persistent, hence it cannot be duplicated and given to multiple threads. This is the sense in which this specification is sequential.
...
\begin{align*}
  \isqueueseq& \; \vq \; \absvalueList \; Q_\gname \eqdef\\
...
For instance, if we wanted to capture the linked list in figure \ref{TLMSQ:impl:figure:enqueue:c}, we would use the list $xs = [(\locinM{3}, \nodevalM{3}, \locoutM{3}); (\locinM{2}, \nodevalM{2}, \locoutM{2});  (\locinM{1}, \nodevalM{1}, \locoutM{1})]$. $\isLL{xs}$ will expand to $\locoutM{3} \mapsto \None \star \isLLchain{xs}$, and $\isLLchain{xs}$ expands to
\begin{align*}
...
Finally, we release the lock using the release specification (and we simply give back $\TRUE$), and the only thing left is to prove the postcondition: $\isqueueseq\ \vq\ (\absvalue :: \absvalueList)\ Q_\gname$. For the existentials, we shall pick the ones we got from the precondition, with the exception for $\xsqueue$ and $\nodetail$. For $\xsqueue$, we shall use the same $\xsqueue$ we got from the precondition, but with $\nodenew$ cons'ed to it, and for $\nodetail$, we chose the new tail node: $\nodenew$. With these choices, proving $\isqueueseq\ \vq\ (\absvalue :: \absvalueList)\ Q_\gname$ is fairly straightforward.
\end{proof}
...
In the next line, we make the head pointer, $\lochead$ point to $\nodeheadnext$, and we have the resource to do this through $\isqueueseq \; \vq \; \absvalueList \; Q_\gname$.\\
...
\begin{align*}
  ...
  &\All \Phi : \Val \to \Prop.\\
  &\quad\quad \All \vq, Q_\gname . \isqueueconc \ \Phi \ \vq \ Q_\gname \implies \persistently \isqueueconc \ \Phi \ \vq \ Q_\gname\\
...
\begin{align*}
  ...
  &\vq = \locqueue \star \locqueue \mapsto^{\persistently} ((\lochead, \loctail), (\Hlock, \Tlock)) \star\\
	&\knowInv{\Nl.queue}{queue\_invariant \Phi \lochead \loctail \Qg} \star\\
...
As mentioned in the observations, it is possible for the tail to lag one node behind the head. This insight lead to including the old nodes of the queue in the queue invariant. This addition manifests in the end of the proof of dequeue. When we open the invariant to swing $\lochead$ to the $\nodeheadnext$, we get that the entire linked list is $xs$. After performing the store, we can then close the invariant with the same $xs$ that we opened the queue to, just written differently to signify that $\nodehead$ is now "old", and $\nodeheadnext$ is the new head node. Because of this, we can supply the same predicate concerning the $tail$ that we got when we opened the invariant, since this only mentions $xs$, which remains the same.

Had we not used an $xs_{old}$ and essentially just "forgotten" old nodes, we couldn't have done this. Say that we defined $xs$ as $xs = \xsqueue \catenate [\nodehead]$ instead. Then, once we have to close the invariant, we cannot supply $xs$, which we got when we opened the invariant. Our only choice (due to the fact that $loc_{head}$ must point to $\nodeheadnext$) is to close the invariant with $xs' = \xsqueue = \xsqueue' \catenate [\nodeheadnext]$. However, clearly $xs' \neq xs$, so we cannot supply the same predicate concerning the $tail$ that we got when opening the invariant, since this predicate talks about $xs$, not $xs'$. Now, if we opened the invariant in the state \DequeueState{}, then we could conclude $\isLast \nodetail \ xs'$ from $\isLast \nodetail \ xs$, due to the relationship between $xs$ and $xs'$, and still be able close the invariant. However, if we opened the invariant in state \BothState{}, then we would need to assert $\isSndLast \nodetail \ xs'$ from $\isSndLast \nodetail \ xs$. This is however not provable, since $\isSndLast \nodetail \ xs$ allows for the case where $\xsqueue'$ is empty, which makes $xs' = [\nodeheadnext]$, disallowing us to prove $\isSndLast \nodetail \ xs'$.
...
For an abstract state $\absvalueList$ and a tuple $\Qg$ of type $\Qgnames$, we shall use the notation $\abstractstateauth{\Qg}{\absvalueList}$ for the ownership assertion $\ownGhost{\Qg.\gname_{Abst}}{\authfull (1, \aginj\ \absvalueList)}$, meaning that the authoritative view of the abstract state associated with $\Qg.\gname_{Abst}$ is $\absvalueList$. Similarly we write $\abstractstatefullfrag{\Qg}{\absvalueList}$ for the assertion $\ownGhost{\Qg.\gname_{Abst}}{\authfrag (1, \aginj\ \absvalueList)}$.
...
\begin{align*}
  ...
  &\quad\quad \All \vq, Q_\gname . \isqueue \ \vq \ Q_\gname \implies \persistently \isqueue \ \vq \ Q_\gname\\
...
Our definition of $\isqueue$ is almost the same as $\isqueueconc$, so we only mention the difference here. The full definition can be found in the appendix \todo{appendix}. The difference is that we no longer take the predicate $\Psi$, and the collection of ghost names is now of type $\Qgnames$. Similarly, the queue invariant, $queue\_invariant$, doesn't require the $\Psi$ any more. Finally, the assertion $All \ \absvalueList \ \Psi$ is changed to $\abstractstateauth{\Qg}{\absvalueList}$.
...
\begin{lemma}[Abstract state agree]\label{TLMSQ:hocap:abst:agree}
  ...
  \abstractstateauth{Q_{\gname H}}{\absvalueList} \star \abstractstatefullfrag{Q_{\gname H}}{\absvalueList'} \proves \absvalueList = \absvalueList'$
\end{lemma}
...
\begin{lemma}[Abstract state update]\label{TLMSQ:hocap:abst:update}
  ...
  \abstractstateauth{Q_{\gname H}}{\absvalueList} \star \abstractstatefullfrag{Q_{\gname H}}{\absvalueList'} \proves \abstractstateauth{Q_{\gname H}}{\absvalueList''} \star \abstractstatefullfrag{Q_{\gname H}}{\absvalueList''}$
\end{lemma}
\todo{consider showing proof}
...
\begin{align*}
  \isqueueseq \; \vq \; \absvalueList \; Q_{\gname S} \eqdef
    &\Exists Q_{\gname H} \in \Qgnames .\\
    &proj\_Qgnames\_seq \ Q_{\gname H} = Q_{\gname S} \star\\
    &\isqueue \ \vq \ Q_{\gname H} \star\\
    &\abstractstatefullfrag{Q_{\gname H}}{\absvalueList}
\end{align*}
\end{definition}
...
We choose $P \eqdef \abstractstatefullfrag{Q_{\gname H}}{\absvalueList}$ and $Q \eqdef \abstractstatefullfrag{Q_{\gname H}}{(\absvalue :: \absvalueList)}$. Note that with this choice, the hoare triple we get after proving the viewshift almost matches the hoare triple we have to prove. The main thing we need is $\isqueue\ \vq \ Q_{\gname H}$ in the postcondition. However, since $\isqueue$ is persistent, and it is present in the precondition, we may assume it in the postcondition. Hence, all we have to prove is the viewshift:
\begin{equation*}
  \All \absvalueList' . \abstractstateauth{\Qg}{\absvalueList'} \star \abstractstatefullfrag{Q_{\gname H}}{\absvalueList} \vs[\mask\setminus\Nl.i^\uparrow] \later \abstractstateauth{\Qg}{\absvalue :: \absvalueList'} \star \abstractstatefullfrag{Q_{\gname H}}{(\absvalue :: \absvalueList)}
\end{equation*}
So assume some $\absvalueList'$, $\abstractstateauth{\Qg}{\absvalueList'}$ and $\abstractstatefullfrag{Q_{\gname H}}{\absvalueList}$. We must then prove $\pvs[\mask\setminus\Nl.i^\uparrow] \later \abstractstateauth{\Qg}{(\absvalue :: \absvalueList')} \star \abstractstatefullfrag{Q_{\gname H}}{(\absvalue :: \absvalueList)}$
...
We use a similar approach to above to prove the \textbf{sequential dequeue spec}. So we assume some $\vq$, $\absvalueList$, and $Q_{\gname S}$, and must then prove the hoare-triple concerning the expression: $\dequeue\ \vq$. This time, we use the hocap-style dequeue spec, with the following choices: $P \eqdef \abstractstatefullfrag{Q_{\gname H}}{\absvalueList}$, and $Q \ \nodeval \eqdef (\absvalueList = [] \star \nodeval = \None \star \abstractstatefullfrag{Q_{\gname H}}{\absvalueList}) \lor (\Exists \absvalue, \absvalueList'. \absvalueList = \absvalueList' \catenate [\absvalue] \star \nodeval = \Some{\absvalue} \star \abstractstatefullfrag{Q_{\gname H}}{\absvalueList'})$.
...
Remember that we need the $\isqueueconc$ predicate to be persistent, hence we cannot simply assert $\abstractstatefullfrag{Q_{\gname H}}{\absvalueList}$ as we did for $\isqueueseq$. Instead we will put it into an invariant. The predicate we will use looks as follows.
\begin{definition}[\tlmsq - $\isqueueconc$ Predicate (Derive)]\label{TLMSQ:spec:isqueueconc_derive}
\begin{align*}
  \isqueueconc \; \vq \; \absvalueList \; Q_{\gname C} \eqdef
  &\Exists Q_{\gname H} \in \Qgnames .\\
  &proj\_Qgnames\_conc \ Q_{\gname H} = Q_{\gname C} \star\\
  &\isqueue \ \vq \ Q_{\gname H} \star\\
  &\knowInv{\Nl.c}{\Exists \absvalueList. \abstractstatefullfrag{Q_{\gname H}}{\absvalueList} \star All\ \absvalueList\ \Psi}
\end{align*}
\end{definition}
...
The \textbf{concurrent initialise spec} follows from the hocap-style initialise spec, after allocating the invariant in the post-condition. We achieve this by applying the rules Ht-csq-vs and inv-alloc, to put the assertions $\abstractstatefullfrag{Q_{\gname H}}{\absvalueList}$ and $All\ [] \Psi$ (which is trivially true) in the postcondition of the hocap style spec into an invariant.

Next, to prove the \textbf{concurrent enqueue spec}, we assume some $\vq$, $\absvalue$, and $Q_{\gname C}$, and must then prove the hoare triple concerning the expression: $\enqueue\ \vq\ \absvalue$. We specialise the hocap-style enqueue spec with $P \eqdef \Psi \ \absvalue$ and $Q \eqdef \TRUE$. The hoare triple we get after proving the viewshift matches the hoare triple we must prove, except that its precondition is weaker: it doesn't mention the invariant or the equality. Hence, the hoare triple we have to prove simply follows by the rule of consequence.
...
\begin{definition}[\lfmsq Invariant]\label{LFMSQ:spec:invariant}
  \begin{align*}
    &queue\_invariant \ \lochead \ \loctail \ Q_\gname \eqdef\\
    &\Exists \absvalueList. \abstractstateauth{Q_\gname}{\absvalueList} \star &&\text{(abstract state)}\\
    ...
    &\lochead \mapsto \nIn{\nodehead} \star\\
    &\loctail \mapsto \nIn{\nodetail} \star\\
    &\ap{Q_\gname.\gname_{Head}}{\nodehead} \star \ar{\nodehead}{Q_\gname.\gname_{Tail}} \star\\
    &\ap{Q_\gname.\gname_{Tail}}{\nodetail} \star \ar{\nodetail}{Q_\gname.\gname_{Last}} \star\\
    &\ap{Q_\gname.\gname_{Last}}{\nodelast}
  \end{align*}
\end{definition}
...
\begin{definition}[\lfmsq - $\isqueue$ Predicate]\label{LFMSQ:spec:isqueue}
  \begin{align*}
    ...
    &\knowInv{\Nl.queue}{queue\_invariant \lochead \loctail \Qg}
  \end{align*}
\end{definition}
...
\end{lemma}
\begin{proof}
We first step through line \ref{LFI:nod} which creates a new node: $x_1 = (\locinM{1}, \None, \locoutM{1})$, with $\locoutM{1} \mapsto \None$ and $\locinM{1} \mapsto (\None, \locoutM{1})$, the latter of which we make persistent. Next, we step through line \ref{LFI:que} which gives us some locations $\lochead$, $\loctail$, and $\locqueue$, with $\lochead \mapsto x_1$ and $\loctail \mapsto x_1$, and finally $\locqueue \mapsto (\lochead, \loctail)$.\\
...
\begin{lemma}[Swing Tail]\label{LFMSQ:spec:swingtail}
  $\All \lochead, \loctail, \nodetail, \nodenewtail, \Qg .\\
  \hoareV[t]{\knowInv{\Nl.queue}{queue\_invariant \ \lochead \ \loctail \ \Qg} \star \reach{\nodetail}{\nodenewtail} \star \ar{\nodenewtail}{\Qg.\gname_{Last}}}{\CAS \ \loctail \ \nIn{\nodetail} \ \nIn{\nodenewtail}}{\valB . \valB = \True \lor \valB = \False}$
\end{lemma}
\begin{proof}
...
\begin{proof}
  ...
  \begin{align}
    &\ar{\nodetail}{\Qg.\gname_{Last}}\label{LFMSQ:spec:proof:enqueue:xtail_ar_last}\\
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      \item[\textbf{Case}]
      $\nodetail = \nodelast'$. We now know that $\nodetail$ is still the last node in the linked list, hence $\nOut{\nodetail} \mapsto \None$, and the $\CAS$ will succeed. This instruction makes $\nodetail$ point to $\nodenew$, which essentially adds it to the linked list. Thus, the value in $\nodenew$ becomes enqueued. In other words, this is a linearisation point, so we must apply the viewshift. We instantiate the viewshift with the abstract state of the queue $\absvalueList$ from the invariant opening, and supply $\abstractstateauth{Q_\gname}{\absvalueList}$ from the invariant and the $P$ from the pre-condition. We hence get $\abstractstateauth{Q_\gname}{\absvalue :: \absvalueList}$ and $Q$.\\
      When closing the invariant, we use $(\absvalue :: \absvalueList)$ for the abstract state, $(\nodenew :: xs)$ for the concrete state, $\nodenew :: \xsqueue$ for the queue, and we take $\nodenew$ to be the last node. The head and tail nodes remain the same. This means we give up $\abstractstateauth{Q_\gname}{\absvalue :: \absvalueList}$ that we got from the viewshift, and the points-to predicate \ref{LFMSQ:spec:proof:xnew2} (used to assert $\isLL (\nodenew :: xs))$. The only thing left to prove is $\ap{\Qg.\gname_{Last}}{\nodenew}$. From the invariant opening, we have $\ap{\Qg.\gname_{Last}}{\nodetail}$. Since $\reach{\nodetail}{\nodenew}$, we may apply lemma \ref{lemma:abs-reach-advance} to update the abstract points-to resource to $\ap{\Qg.\gname_{Last}}{\nodenew}$ and additionally obtain $\ar{\nodenew}{\Qg.\gname_{Last}}$. With this, we can close the invariant, and step to line \ref{LFE:sts}. This line attempts to swing the tail, so we apply our swing-tail lemma (lemma \ref{LFMSQ:spec:swingtail}) by supplying our invariant, $\reach{\nodetail}{\nodenew}$, and $\ar{\nodenew}{\Qg.\gname_{Last}}$. This tells us that the $\CAS$ is safe, and it either succeeds or fails. The resulting value is the returned value of the $\enqueue$ function, but since the post-condition is simply $Q$, which we own, we are done.
...
\begin{proof}
  ...
  \begin{align}
    ...
    &\ar{\nodehead}{\Qg.\gname_{Head}}\label{LFMSQ:spec:proof:dequeue:xhead_ghead}\\
    &\ar{\nodehead}{\Qg.\gname_{Tail}}\label{LFMSQ:spec:proof:dequeue:xhead_gtail}\\
    &\ar{\nodehead}{\Qg.\gname_{Last}}\label{LFMSQ:spec:proof:dequeue:xhead_glast}
  \end{align}
  ...
  \begin{align}
    ...
    &\ar{\nodetail}{\Qg.\gname_{Tail}}\label{LFMSQ:spec:proof:dequeue:xtail_gtail}\\
  \end{align}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \begin{align}
        ...
        &\ar{\nodeheadnext}{\Qg.\gname_{Last}}\label{LFMSQ:spec:proof:dequeue:x_headnext_glast}
      \end{align}
