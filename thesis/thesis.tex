...
\begin{align*}
  \isqueueconc \; v_q \; xs_v \; \Qg = &\exists \loc_{queue}, \loc_{head}, \loc_{tail} \in \Loc . \exists h_{lock}, t_{lock} \in \Val . \\
  &v_q = \loc_{queue} \ast \loc_{queue} \mapsto^{\persistently} ((\loc_{head}, \loc_{tail}), (h_{lock}, t_{lock})) \ast\\
...
\begin{align*}
  ...
  &\land\quad \forall v_q, v, Q_\gname, P, Q.
    \begin{array}[t]{l}
    \left(\forall xs_v . \abstractstateauth{\Qg}{xs_v} \ast P \vs[\mask\setminus\Nl.i^\uparrow] \later \abstractstateauth{\Qg}{v :: xs_v} \ast Q \right)
    \wand\\
    \hoare{\isqueue \ v_q \ Q_\gname \ast P}{\enqueue\ v_q\ v}{v . Q}
    \end{array}\\
  &\land\quad \forall v_q, Q_\gname, P, Q.
    \begin{array}[t]{l}
    \left(\forall xs_v . \abstractstateauth{\Qg}{xs_v} \ast P \vs[\mask\setminus\Nl.i^\uparrow] \later 
      \left( 
        \begin{array}{l}
...
\todo{text. also write about the two lemmas on the abstract state that we rely on.}
