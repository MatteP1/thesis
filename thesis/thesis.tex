...
In this section, we look at an example queue client, named \textit{QueueAdd}. The example is somewhat contrived but illustrates the capabilities of the HOCAP-style specification, and the limitations of the concurrent and sequential specifications.
...
\begin{definition}[QueueAdd Specification]\label{QueueSpecs:spec:queueadd}
  ...
  \end{equation*}
\end{definition}
This also implies that the program does not crash; the first case in the $\unwrap$ function never occurs.

Note first that, if we had not used the parallel composition, and simply executed $\enqdeq$ twice in sequence, then the sequential specification would have sufficed in proving the specification. We would always have the $\isqueueseq$ predicate at hand to step through the $\enqueue$ and $\dequeue$ in the $\enqdeq$ function, and this would allow us to track the exact contents, which means that we could deduce that we never invoke $\dequeue$ when the queue is empty, and the values dequeued are exactly $a$ and $b$. However, since $\isqueueseq$ is not persistent (and hence not duplicable), it is not strong enough for the version using parallel composition.\\
The specification for parallel composition, \ruleref{Ht-par}, states that we must decide which resources we give to each thread. We can not give both threads $\isqueueseq$ as it is not duplicable, but both threads will need the predicate in order to apply the specifications for $\enqueue$ and $\dequeue$.

\begin{mathpar}
  \Htpar
\end{mathpar}

The Concurrent specification does not have this issue, as its queue predicate, $\isqueueconc$ \textit{is} persistent. However, the concurrent specification is not strong enough as it does not track the state of the queue. It is paramount for correctness that the $\dequeue$ in $\enqdeq$ does not result in $\None$; the $\unwrap$ crashes if it does. However, the concurrent specification only tells us that the $\dequeue$ is either $\None$ or $\Some \val$, for some value $\val$. We have no way of proving that it must the latter case, as we do not track the contents of the queue.\\
Fortunately, the HOCAP-style specification allows us to track the contents of the queue, so we are able to exclude the cases where the dequeues result in $\None$. Further, we can argue about the actual values that are dequeued; it must be either a or b. The way we realise this, is by creating an invariant that expresses the \textit{five} possible states of the queue using the fragmental view of the abstract state, which we get from the HOCAP-style $\initialise$ specification. The invariant looks as follows.
\begin{definition}[Invariant for QueueAdd]\label{QueueSpecs:queueadd:invariant}
  ...
  \end{align*}
\end{definition}
Since invariants are persistent, we can give both the invariant and the HOCAP-style queue predicate to both threads, meaning that both threads can use the HOCAP-style specifications for $\enqueue$ and $\dequeue$, when stepping through $\enqdeq$. The threads can open the invariant to get the fragmental view which they can use to prove the required view-shifts of the specifications. The invariant also mentions four \textit{tokens}, $\TokDoQAg$, $\TokDtQAg$, $\TokAQAg$, and $\TokBQAg$, which the threads use to reason about which state the queue is in when they open the invariant (see \ref{RA:sections:tokens} for a discussion on tokens). In particular, both threads can reason that, after its enqueue, it cannot be in the first state of the invariant, which in particular means that the queue cannot be empty. Hence they can rule out the case where the dequeue results in $\None$, and they can argue that the value dequeued is either $a$ or $b$. Further, using the tokens, we can also argue that one thread must have dequeued $a$ and the other $b$; they could not have dequeue the same value.\\
We omit the precise proof here, and refer instead to the formal proof found in \path{queue_client.v}.
