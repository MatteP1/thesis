...
The proposition $All \ xs_v \ \Phi \ast$ states that all values in $xs_v$ (i.e. the values currently in the queue) satisfy the predicate $\Phi$. This will allow us to conclude that dequeued values satisfy $\Phi$.\\
...
\begin{align*}
  \isqueue \; q \; xs_v \; \Qgnames = &\exists \loc_{queue}, \loc_{head}, \loc_{tail} \in \Loc . \exists H_{lock}, T_{lock} \in \Val . \\
  &q = \loc_{queue} \ast \loc_{queue} \mapsto\persistently ((\loc_{head}, \loc_{tail}), (H_{lock}, T_{lock})) \ast\\
