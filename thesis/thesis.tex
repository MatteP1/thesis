...
\begin{lemma}[Lock-Free M\&S-Queue Specification - Initialise]\label{LFMSQ:spec:init}
  \begin{equation*}
    \hoare{\TRUE}{\initialise \TT}{v_q . \Exists Q_\gname . \isqueue\ v_q \ Q_\gname \star \abstractstatefullfrag{\Qg}{[]}}
  \end{equation*}
\end{lemma}
\begin{proof}
...
\begin{proof}
  ...
  \begin{equation}\label{LFMSQ:spec:proof:isqueue}
  ...
  \begin{equation}\label{LFMSQ:spec:proof:xnew}
    \nIn{x_{new}} \mapsto^{\persistently} (\Some v, \nOut{x_{new}}) \star \nOut{x_{new}} \mapsto \None
  \end{equation}
  ...
  Line \ref{LFE:tal} first dereferences to the tail pointer $\loc_{tail}$ using the resources in \ref{LFMSQ:spec:proof:isqueue}. We open the invariant to obtain the points-to predicate concerning $\loc_{tail}$. We get that $\loc_{tail}$ points to some $x_{tail}$. Using the resources from the invariant, we may conclude the following persistent information:
  \begin{equation}\label{LFMSQ:spec:proof:xtail}
    \ar{x_{tail}}{\Qg.\gname_{Last}} \star \nIn{x_{tail}} \mapsto^{\persistently} (\nVal{x_{tail}}, \nOut{x_{tail}})
  \end{equation}
...
\subsubsection{Dequeue}
\begin{lemma}[Lock-Free M\&S-Queue Specification - Enqueue]\label{LFMSQ:spec:dequeue}
  \begin{equation*}
    \All v_q, Q_\gname, P, Q.
    \begin{array}[t]{l}
    \left(\All xs_v . \abstractstateauth{\Qg}{xs_v} \star P \vs[\mask\setminus\Nl.i^\uparrow] \later
      \left(
        \begin{array}{l}
