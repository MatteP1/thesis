...
- Derivation rules
- HeapLang
- Syntactic sugar (i.e. e1 ;; e2 = (lam v, e2) e1 where v is fresh, and CAS ... as Snd (CMPXHG ...)), and derived rules for them.
...
\todo{fix notation}
\begin{lemma}
  $\reach{x_n}{x_m} \ast-\ast \isNode{x_n}$
\end{lemma}

\todo{fix notation}
\begin{lemma}
  $\reach{x_n}{x_m} -\ast \reach{x_m}{x_o} -\ast \reach{x_n}{x_o}$
\end{lemma}

\begin{lemma}
  $\reach{x_n}{x_m} -\ast \isNode{x_n}$
\end{lemma}

\begin{lemma}
  $\reach{x_n}{x_m} -\ast \isNode{x_m}$
\end{lemma}

\begin{lemma}
  $\reach{x_n}{x_m} -\ast \nOut{x_n} \mapsto \None -\ast x_n = x_m \ast \nOut{x_n} \mapsto \None$
\end{lemma}
