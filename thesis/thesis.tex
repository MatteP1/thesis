...
\newcommand{\Qgnamesseq}{Q_{\gname S}}
\newcommand{\Qgnamesconc}{Q_{\gname C}}
\newcommand{\Qgnameshocap}{Q_{\gname H}}
...
\begin{align*}
  ...
  &\land\quad\forall v_q, v, xs_v, Q_\gname. \hoare{\isqueueseq \; v_q \; xs_v \; Q_\gname}{\enqueue\ v_q\ v}{w . \isqueueseq\ v_q\ (v :: xs_v)\ Q_\gname}\\
  &\land\quad\forall v_q, xs_v, Q_\gname. \hoareV[t]{\isqueueseq \; v_q \; xs_v \; Q_\gname}{\dequeue\ v_q}{v . \begin{array}{l}(xs_v = [] \ast v = \None \ast \isqueueseq\ v_q\ xs_v\ Q_\gname) \; \lor\\ (\exists x_v, xs_v' \ . \ xs_v = xs_v' \catenate [x_v] \ast v = \Some x_v \ast \isqueueseq\ v_q\ xs_v'\ Q_\gname) \end{array}}
\end{align*}
...
\begin{align*}
  ...
  &\quad\quad \forall v_q, Q_\gname . \isqueueconc \ \Phi \ v_q \ Q_\gname \implies \persistently \isqueueconc \ \Phi \ v_q \ Q_\gname\\
  &\land\quad\hoare{\TRUE}{\initialise \TT}{v_q . \exists Q_\gname, \isqueueconc\ \Phi \ v_q \ Q_\gname}\\
  &\land\quad\forall v_q, v, Q_\gname. \hoare{\isqueueconc \ \Phi \ v_q \ Q_\gname \ast \Phi \ v}{\enqueue\ v_q\ v}{v . \TRUE}\\
  &\land\quad\forall v_q, Q_\gname. \hoare{\isqueueconc \ \Phi \ v_q \ Q_\gname}{\dequeue\ v_q}{v . v = \None \lor (\exists x_v, v = \Some x_v \ast \Phi \ x_v)}
\end{align*}
...
\begin{align*}
  \isqueueconc \; v_q \; xs_v \; \Qgnames = &\exists \loc_{queue}, \loc_{head}, \loc_{tail} \in \Loc . \exists h_{lock}, t_{lock} \in \Val . \\
  &v_q = \loc_{queue} \ast \loc_{queue} \mapsto^{\persistently} ((\loc_{head}, \loc_{tail}), (h_{lock}, t_{lock})) \ast\\
...
For an abstract state $xs_v$ and a tuple $\Qgnames$ of type $Qgnames$, we shall use the notation $\abstractstateauth{\Qgnames}{xs_v}$ for the ownership assertion $\ownGhost{\Qgnames.\gname_{Abst}}{\authfull (1, xs_v)}$, meaning that the authoritative view of the abstract state associated with $\Qgnames.\gname_{Abst}$ is $xs_v$. Similarly we write $\abstractstatefullfrag{\Qgnames}{xs_v}$ for the assertion $\ownGhost{\Qgnames.\gname_{Abst}}{\authfrag (1, xs_v)}$.

With this, we now give the hocap-style spec, and explain it afterwards. 
\begin{align*}
  &\exists \isqueue : \Val \to Qgnames \to \Prop.\\
  &\quad\quad \forall v_q, Q_\gname . \isqueue \ v_q \ Q_\gname \implies \persistently \isqueue \ v_q \ Q_\gname\\
  &\land\quad\hoare{\TRUE}{\initialise \TT}{v_q . \exists Q_\gname, \isqueue\ v_q \ Q_\gname \ast \abstractstatefullfrag{\Qgnames}{[]} }\\
  &\land\quad \forall v_q, v, Q_\gname, P, Q.
    \begin{array}[t]{l}
    \left(\forall xs_v . \abstractstateauth{\Qgnames}{xs_v} \ast P \vs[\mask\setminus\Nl.i^\uparrow] \later \abstractstateauth{\Qgnames}{v :: xs_v} \ast Q \right)
    \wand\\
    \hoare{\isqueue \ v_q \ Q_\gname \ast P}{\enqueue\ v_q\ v}{v . Q}
    \end{array}\\
  &\land\quad \forall v_q, Q_\gname, P, Q.
    \begin{array}[t]{l}
    \left(\forall xs_v . \abstractstateauth{\Qgnames}{xs_v} \ast P \vs[\mask\setminus\Nl.i^\uparrow] \later 
      \left( 
        \begin{array}{l}
          (xs_v = [] \ast \abstractstateauth{\Qgnames}{xs_v} \ast Q \ \None)\\
          \lor
          \left(
            \begin{array}{l}
              \exists x_v, xs_v' . xs_v = xs_v' \catenate [x_v] \ast\\
              \abstractstateauth{\Qgnames}{xs_v'} \ast Q \ (\Some{x_v})
            \end{array}
            \right)
        \end{array}
      \right) \right)
...
Next, the initialise spec gives clients an additional resource in the postcondition: the ownership of the fragmental view of the empty list, $\abstractstatefullfrag{\Qgnames}{[]}$. As discussed above, this allows them to keep track of the contents of the queue.
...
For initialise, we must additionally get ownership of the authoritative and fragmental view of the abstract state, both of which should state that it is empty. I.e. we must get $\abstractstateauth{\Qgnames}{[]} \ast \abstractstatefullfrag{\Qgnames}{[]}$. We achieve this by own-op and and own-allocate, which requires us to show that $\authfull (1, []) \cdot \authfrag (1, []) \in \Vl$. This follows by the definitions of the resource algebras. We use $\abstractstateauth{\Qgnames}{[]}$ to establish the queue invariant, and $\abstractstatefullfrag{\Qgnames}{[]}$ to prove the post-condition.\\
...
\textbf{Enqueue} We start by assuming the viewshift which allows us to update $P$ to $Q$ and $\abstractstateauth{\Qgnames}{xs_v}$ to $\abstractstateauth{\Qgnames}{v :: xs_v}$, for any $xs_v$. We must then prove the hoare triple for the expression $\enqueue \ v_q \ v$. The only real change from the previous proof happens the second time we open the invariant; the first and third times, the abstract state doesn't change, hence we can simply frame away the newly added authoritative fragment concerning the abstract state, and continue as we did before. The second time we open the invariant, it is around the expression that adds the newly created node to the linked list (\todo{add line number}). When opening it, we get $\abstractstateauth{\Qgnames}{xs_v}$. As before, we also get all the resources to match up variables and step through the code, updating the concrete state. To close the invariant, we must make the same choice of abstract state as we did previously: $v :: xs_v$. This, however, requires us to obtain $\abstractstateauth{\Qgnames}{v :: xs_v}$. However, since we have $\abstractstateauth{\Qgnames}{xs_v}$ and $P$ (from the precondition), we can apply the viewshift to obtain it, along with $Q$. This then allows us to close the invariant, and the proof proceeds as previously. At the end, we must also prove the postcondition $Q$, but this is no issue as we obtained that from the viewshift.

\textbf{Dequeue} \todo{text}
