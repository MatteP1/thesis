...
As before, we collect the ghost names we will need in a tuple, this time of type $Qgnames$. It is similar to $ConcQgnames$, but with one additional ghost name: $\gname_{Abst}$ for elements in the resource algebra we constructed above.
For an abstract state $xs_v$ and a tuple $\Qgnames$ of type $Qgnames$, we shall write \todo{insert notation} to denote ownership of the element \todo{insert raw element}, meaning that the authoritative view of the abstract state associated with $\Qgnames.\gname_{Abst}$ is $xs_v$. Similarly we write \todo{insert notation} for the element \todo{insert raw element}.
